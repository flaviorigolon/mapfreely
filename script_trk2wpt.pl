...
#  usa gpx2shp per convertire il GPX di track creato in SHP
   system("gpx2shp -t -o track_$nome_ref.shp $nome_ref.gpx");

#  usa gpx2shp per convertire il GPX di waypoint creato in SHP
...
#  usa ogr2ogr per riproiettare lo shape di waypoint creato da EPSG 4326 (lat-long) a EPSG 3003 (Gauss-Boaga Fuso Ovest)
...
#  usa ogr2ogr per riproiettare lo shape di track creato da EPSG 4326 (lat-long) a EPSG 3003 (Gauss-Boaga Fuso Ovest)
   system("ogr2ogr gb_track_$nome_ref.shp track_$nome_ref.shp -s_srs EPSG:4326 -t_srs '+proj=tmerc +ellps=intl +lat_0=0 +lon_0=9 +k=0.999600
+x_0=1500000 +y_0=0 +units=m +towgs84=-104.1,-49.1,-9.9,0.971,-2.917,0.714,-11.68'");

#  usa ogr2ogr per convertire lo shape track creato (in wgs84) in GML per essere caricato mediante Openlayers
   system("ogr2ogr -f GML track_$nome_ref.gml track_$nome_ref.shp");
}